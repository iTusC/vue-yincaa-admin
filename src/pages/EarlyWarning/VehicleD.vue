        ...
        <section class="cd-echart">
          <div class="cd-eachart-title">
            ...
            <el-col :span="5" :offset="9">
              <el-cascader :options="onlineOptions" v-model="echartsOptionsModel" @change="echartshandleChange" @active-item-change="echartshandleItemChange" size="mini" :props="onlineProps" v-if="onlineS">
              </el-cascader>
            </el-col>
          </div>
          ...
          <div class="legend" ref="legend">
            <p>
              <span></span>时间：{{ dataTime }}</p>
            <p>
              <span></span>预警总次数{{ dataNumber }}次</p>
          </div>
...
export default {
    ...
    methods: {
        ...
        async getComposAlarmCount(params) {
            let response = await vehicleAlarmCount(params);
            let res = response.data;
            this.eChartsPostion();
            this.valueCode = this.echartsOptionsModel[1];
            if (res.code == 0) {
