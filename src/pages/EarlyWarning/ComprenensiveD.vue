          ...
          <div class="cd-online-title">
            <el-row :gutter="20">
              ...
              <el-col :span="8" :offset="6">
                <el-cascader :options="onlineOptions" v-model="selectedOptions" @change="handleChange" @active-item-change="handleItemChange" size="mini" :props="onlineProps" v-if="onlineS">
                </el-cascader>
              </el-col>
            </el-row>
          </div>
          <div class="cd-online-main">
            ...
            <template v-if="onlineD">
              <div class="online-info">
                <p>
                  <span :class="rate > 0.5 || rate == 0.5 || onlineone ? 'infoGreen' : 'infoRed'" class="info-sty"></span>
                  <i class="info-name">在线车辆：</i>
                  <i class="info-car-number">{{ onlineRate.onLine }}</i>
                  <i class="info-car">辆</i>
                </p>
                <p>
                  <i class="info-car-all">所有车辆：</i>
                  <i class="info-car-numbers">{{ onlineRate.count }} 辆</i>
                </p>
                <p>
                  <i class="info-car-all">离线车辆：</i>
                  <i class="info-car-numbers">{{ onlineRate.offLine }} 辆</i>
                </p>
              </div>
            </template>
            <template v-else>
              <div class="online-info">
                <p>
                  <span :class="deriverisOnline === 1? 'infoGreen' : 'infooff'" class="info-sty"></span>
                  <i class="info-name">在线情况：</i>
                  <i class="info-car-number" style="width:50px;" v-text="deriverisOnline === 1?'在线':'离线'"></i>
                  <i class="info-car"></i>
                </p>
                <p>
                  <i class="info-car-all">所属车队：</i>
                  <i class="info-car-numbers">{{ dName }}</i>
                </p>
                <p>
                  <i class="info-car-all">车牌号码：</i>
                  <i class="info-car-numbers">{{ vehicleNumber }}</i>
                </p>
                <p>
                  <i class="info-car-all">驾驶人数：</i>
                  <i class="info-car-numbers">{{ deriverSum }} 人</i>
                </p>
              </div>
            </template>
        ...
        <section class="cd-echart">
          <div class="cd-eachart-title">
            ...
            <el-col :span="5" :offset="9">
              <el-cascader :options="echartsOptions" v-model="echartsOptionsModel" @change="echartshandleChange" @active-item-change="echartshandleItemChange" size="mini" :props="echartsProps" v-if="onlineS">
              </el-cascader>
            </el-col>
          </div>
          <table>
            <tr>
              <td>
                <div ref="legend" class="legend">
                  <p v-if="ishowDLegend" class="legendD">
                    <span></span>车辆预警</p>
                  <p v-if="ishowVLegend" class="legendV">
                    <span></span>驾驶员预警</p>
                </div>
              </td>
            </tr>
                ...
                <el-col :span="12">
                  ...
                  <el-select v-if="onlineS" @change="getAlarmCode" v-model="value7" placeholder="请选择" size="mini">
                    <el-option-group size="mini" v-for="group in options3" :key="group.label" :label="group.label">
                      <el-option v-for="item in group.options" :key="item.atypeName" :label="item.atypeName" :value="item.atypeId">
                      </el-option>
                    </el-option-group>
                  </el-select>
                </el-col>
...
export default {
    ...
    methods: {
        ...
        showFun(msg) {
            ...
            //获取预警列表预警类型value
            this.getByGategory({pId: 64}, {pId: 65});
        },
        ...
        itemOpened(msg) {
            ...
            //点击查询车辆在线情况
            this.getTeamTree(
                {companyId: this.companyCode, searchCondition: text},
                true
            );
            //综合echarts,点击车辆查询统计
        ...
        listhandleItemChange(val) {
            ...
            let compId = Number(this.companyCode);
            //判断时间筛选是否为真
            if (this.tabletimestart || this.tabletimeend) {
                this.getAlarmCompsStat({
                    pageNum: this.pageNumber,
        ...
        showList(i) {
            ...
            if (this.tableListShowi === i) {
                ...
                this.getAlarmCompsDetail({
                    ...
                    deriverId: this.tableData[i].driverId,
                    startDate: this.starData,
                    endDate: this.endData,
                });
        ...
        getCookie: function() {
            if (document.cookie.length > 0) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    if (arr2[0] == 'companyId') {
                        this.companyCode = Number(arr2[1]); //保存到保存数据的地方
                    } else if (arr2[0] == 'companyName') {
                        this.companyName = arr2[1];
                    } else if (arr2[0] == 'imgPath') {
                        this.companyImagePath =
                            'http://47.106.196.228:8888/' + arr2[1];
                    }
                }
            }
        },
        ...
        async getTeamTree(params, select = false, vehicle = false) {
            let response = await teamTree(params);
            let res = response.data;
            if (res.code == 0) {
                if (select) {
                    this.dName = res.data.teams[0].teamName;
                    this.deriverSum =
                        res.data.teams[0].vehicleList[0].driverCount;
                    this.deriverisOnline =
                        res.data.teams[0].vehicleList[0].isOnline;
                    this.onlineone = res.data.teams[0].vehicleList[0].isOnline;
                    this.vehicleNumber =
                        res.data.teams[0].vehicleList[0].vehicleCode;
                } else if (vehicle) {
                    this.onlineOptions = res.data.teams;
                    this.onlineOptions[0].onlineChr =
                        res.data.teams[0].vehicleList;
                    this.echartsOptions = res.data.teams[0].vehicleList;
                    ...
                    this.vehicleList = res.data.teams;
                    this.selectNumber = res.data.teams.length;
                }
            }
            res = null;
        },
        ...
        async getSearchByName(params) {
            let response = await searchByName(params);
            let res = response.data;
            let data = [];
            if (res.code == 0) {
                for (let i = 0; i < res.data.dropDownList.length; i++) {
                    data.push({value: res.data.dropDownList[i].searchResult});
                }
                this.resL = data;
            }
            res = null;
        ...
        async getOnlineRateAll(params, vehicle = false) {
            let response = await onlineRateAll(params);
            let res = response.data;
            //判断是否是车辆查询在线数
            if (res.code == 0) {
                if (vehicle) {
        ...
        async getComposAlarmCount(params, vehicle = false, divers = false) {
            let response = await composAlarmCount(params);
            let res = response.data;
            this.eChartsPostion();
            if (res.code == 0) {
                res.data.driverAlarmCount.forEach(element => {
                    this.polar.xAxis.data.push(
                        element.everyDater.slice(5).replace(/-/, '/')
                    );
                });
                res.data.driverAlarmCount.forEach(element => {
                    this.polar.series[0].data.push(
                        Object.freeze(element.alarmCount)
                    );
                });
                res.data.vehicleAlarmCount.forEach(element => {
        ...
        async getAlarmCompsDetail(param) {
            ...
            let response = await alarmCompsDetails(param);
            let res = response.data;
            if (res.code === 0) {
        ...
        async getByGategory(params, i) {
            this.loading = true;
            let response = await ByGategory(params);
            let res = response.data;
            if (res.code === 0) {
                let arr = [];
                for (let i = 0; i < res.data.alarm.length; i++) {
                    if (
                        res.data.alarm[i].atypeParent &&
                        res.data.alarm[i].atypeParent == '64'
                    ) {
                        arr.push(res.data.alarm[i]);
                    }
                }
                ...
                if (resp.code == 0) {
                    let arrT = [];
                    for (let i = 0; i < resp.data.alarm.length; i++) {
                        if (
                            resp.data.alarm[i].atypeParent &&
                            resp.data.alarm[i].atypeParent == '65'
                        ) {
                            arrT.push(resp.data.alarm[i]);
                        }
                    }
