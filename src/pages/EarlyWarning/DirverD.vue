      ...
      <div class="yc-cd-mains">
        <section class="yc-dd-main">
          <div class="cd-eachart-title">
            <el-col :span="10">
              <el-row :gutter="20">
                ...
                <el-col :span="5">
                  <el-date-picker v-model="selectData" align="right" type="date" size="mini" placeholder="选择日期" :picker-options="pickerOptions" style="width:150px;" @change="dataMothe" value-format="yyyy-MM-dd" :editable="false">
                  </el-date-picker>
                </el-col>
          ...
          <div ref="legend" class="legend">
            ...
            <p>
              <span></span>预警总次数:{{ sumCount }}次</p>
            <p v-if="vehicleNumbner" v-for="items in driversD">
              <span></span>{{ items.deriverName }}:{{ items.deriverCount }}次</p>
          </div>
...
export default {
    ...
    methods: {
        ...
        async getDeriverAlarmCount(params) {
            let response = await deriverAlarmCount(params);
            let res = response.data;
            this.eChartsPostion();
            if (res.code == 0) {
        ...
        async getDeriverVehicleAlarmCount(params) {
            let response = await deriverVehicleAlarmCount(params);
            let res = response.data;
            this.eChartsPostion();
            if (res.code == 0) {
                res.data.driverDetails[0].alarms.forEach(element => {
                    this.polar.xAxis.data.push(element.aTypeName);
                    for (let i = 0; i < res.data.driverDetails.length; i++) {
                        for (
                            let c = 0;
                            c < res.data.driverDetails[0].alarms.length;
                            c++
                        ) {
                            this.polar.series[i].data.push(
                                res.data.driverDetails[i].alarms[c].alarmCount
                            );
                        }
                        this.polar.series[i].name =
                            res.data.driverDetails[i].alarms.driverName;
                    }
                });
            ...
            if (res.code == 0) {
                ...
                for (let i = 0; i < res.data.alarm.length; i++) {
                    if (
                        ...
                        res.data.alarm[i].atypeParent == '65'
                    ) {
        ...
        async getCompos(params) {
            ...
            if (res.status == 200) {
                ...
                this.companyImage =
                    'http://47.106.196.228:8888/' + res.data.logoPath;
            }
        },
        //获取点击teamcode
